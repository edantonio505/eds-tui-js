...
export interface PasteClassification {
  /** true = collapse to a [+N lines] placeholder; false = insert the trimmed text directly, as if typed. */
  placeholder: boolean;
  /** Non-blank line count — only meaningful when placeholder is true; matches Python's `l.strip()`-filtered count. */
  lineCount: number;
  /** What to actually insert into the visible buffer at the cursor. */
  insertText: string;
}
 ...
 */
export function classifyPaste(rawPastedText: string): PasteClassification {
 ...
 */
export function spliceBackPastes(finalLine: string, pastedBlocks: string[]): string {
  let i = 0;
  const spliced = finalLine.replace(/\[\+\d+ lines\]/g, () => {
    if (i >= pastedBlocks.length) return ""; // more placeholders than stashed blocks shouldn't happen; degrade quietly
...
export interface PromptResult {
  text: string;
  cancelled: boolean; // Ctrl-C
  eof: boolean; // Ctrl-D on an empty buffer
}
 ...
 */
export function promptLine(promptText: string): Promise<PromptResult> {
  return new Promise((resolve) => {
    ...
    function finishPaste(): void {
      ...
      if (result.placeholder) pastedBlocks.push(pasteAccum);
    ...
    function onData(chunk: Buffer): void {
      ...
      while (i < str.length) {
        ...
        if (ch === "\x03") return cancel();
        if (ch === "\x04") {
          if (buf.length === 0) return eof();
          i += 1;
          continue;
        }
        if (ch === "\r" || ch === "\n") return submit();
