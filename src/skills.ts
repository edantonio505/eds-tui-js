...
export const VALID_MODELS: readonly ModelRole[] = ["main", "small", "any"];
...
let problemsList: Array<[string, string]> = [];
...
function loadOne(directory: string): Skill {
  ...
  const model = (meta.model ?? "any").toLowerCase() as ModelRole;
...
export function discover(): Map<string, Skill> {
...
export function problems(): Array<[string, string]> {
  discover();
  return [...problemsList];
}

/** Look up a skill by name, case-insensitively. null if there is no such skill. */
export function get(name: string | null | undefined): Skill | null {
  const found = discover();
  if (name && found.has(name)) return found.get(name)!;
  const lowered = (name ?? "").toLowerCase();
  for (const [key, skill] of found) {
    if (key.toLowerCase() === lowered) return skill;
  }
  return null;
}

/** The name+description index — everything the model knows until a skill loads. */
export function indexLines(subset?: Map<string, Skill> | Record<string, Skill>): string {
  const entries: Skill[] = subset
...
export function match(
  ...
  subset?: Map<string, Skill> | Record<string, Skill>
): Skill | null {
  ...
  for (const name of names) {
    if (lowered.includes(name.toLowerCase())) return found.get(name)!;
  }
  return null;
}

/** A skill as the model receives it, with its directory named so bundled files resolve. */
export function render(skill: Skill): string {
  return (
    `--- skill: ${skill.name} ---\n` +
    `${skill.description}\n` +
 ...
 */
export function write(args: WriteSkillArgs): Skill {
  ...
  const model = ((args.model ?? "any").trim().toLowerCase()) as ModelRole;
  if (!VALID_MODELS.includes(model)) {
    throw new Error(`model must be one of ${VALID_MODELS.join(", ")} — got '${model}'`);
  }
