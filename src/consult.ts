...
export function buildConsultPrompt(cwd: string, appDir: string): string {
  return (
    "You are being consulted by another AI assistant for 'eds tui', a terminal assistant " +
 ...
 */
export function makeConsultSpecialist(
  pool: PoolEntry[],
  smallModel: string,
  cwd: string,
  appDir: string
  ...
  return async (client: Ollama, task: string): Promise<string> => {
    ...
    for (let i = 0; i < CONSULT_MAX_TURNS; i++) {
      let response;
      try {
        response = await ui.withSpinner(`${CONSULT_INDENT}${ui.displayModel(specialist)} working...`, () =>
    ...
    let result = "";
    try {
      const response = await ui.withSpinner(`${CONSULT_INDENT}${ui.displayModel(specialist)} wrapping up...`, () =>
        client.chat({
          model: specialist,
    ...
    if (!result) {
      return (
        `The consult with ${specialist} hit its ${CONSULT_MAX_TURNS}-step limit without a ` +
    ...
    return (
      `Consultation with ${specialist} concluded (hit its ${CONSULT_MAX_TURNS}-step limit; ` +
