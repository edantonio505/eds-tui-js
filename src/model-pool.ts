...
export function loadPool(path: string = MODEL_POOL_FILE): PoolEntry[] {
  ...
  for (const raw of parsed) {
    if (
      ...
      typeof (raw as any).good_for === "string" &&
      (raw as any).good_for.trim()
    ) {
      entries.push({ name: (raw as any).name.trim(), goodFor: (raw as any).good_for.trim() });
    }
  }
  return entries;
}

function poolIndexLines(pool: PoolEntry[]): string {
  return pool.map((p) => `- ${p.name}: ${p.goodFor}`).join("\n");
}

const MODEL_POOL_MATCH_SYSTEM = (index: string): string =>
  "You pick the best specialist model for a request a general-purpose assistant " +
 ...
 */
export async function pickSpecialistModel(
  client: Ollama,
  userInput: string,
  pool: PoolEntry[],
  smallModel: string
  ...
  for (const entry of sorted) {
    if (reply.includes(entry.name.toLowerCase())) return entry.name;
  }
  return null;
}
