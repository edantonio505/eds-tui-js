...
const COMPACTION_SYSTEM = "You compress part of an ongoing terminal-assistant transcript into a short factual " +
 ...
 */
function findTailStart(messages, roundsToKeep) {
    let rounds = 0;
    for (let i = messages.length - 1; i >= 1; i--) {
        if (messages[i]?.role === "assistant") {
            rounds++;
            if (rounds === roundsToKeep)
                return i;
        }
    }
    return 1;
 ...
 * compacted anything.
 */
export async function maybeCompact(client, smallModel, messages) {
    if (totalChars(messages) <= COMPACTION_THRESHOLD_CHARS)
        return false;
    ...
    if (middleEnd - middleStart < MIN_COMPACTABLE_MESSAGES)
        return false;
    ...
    const note = {
        role: "system",
        content: `SYSTEM NOTE: the following summarizes ${middle.length} earlier message(s) from this ` +
